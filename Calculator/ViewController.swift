...
class ViewController: UIViewController, AVAudioPlayerDelegate {
    ...
    @IBAction func numButtonsPressed(_ sender: UIButton) {
        ...
        if (displayNum.text! == "0" || displayNum.text! == "+" || displayNum.text! == "-" || displayNum.text! == "*" || displayNum.text! == "/" || modOccured ) && !(sender.tag == 0) && !(sender.tag==100){
            displayNum.text? = ""
            displayNum.text?.append(String(sender.tag))
        }
    ...
    @IBAction func performOperation(_ sender: UIButton) {
        ...
        if sender.tag == 10 || sender.tag == 4{
          ...
          //  displayNum.text = String(previousNum)
            calculatevalue(value: displayNum.text!)
            previousNum = 0
            preTag = "+"
        }
        else{
            displayNum.text?.append(String(tagList[sender.tag]))
    ...
    func calculatevalue(value: String){
       ...
       let mathExpression = NSExpression(format: value)
        ...
        displayNum.text = String(mathValue!)
